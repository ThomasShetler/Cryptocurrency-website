...
def get_bit(request, name):
    ...
    context = {'response': response.json()['Realtime Currency Exchange Rate'],
               'form': form}
    print('!!!!', context['response']['6. Last Refreshed'])
    print(type(context['response']))
    return render(request, 'CryptoCurrency/crypto_api.html', context)
...
def details(request, pk):
    ...
    info = get_object_or_404(Info, pk=pk)  # Gets single instance of the jersey from the database
...
def add_collection(request):
    ...
    if form.is_valid():                         #Checks the form for errors, to make sure it's filled in
        ...
        return redirect('cryptoList')                #Redirects to the index page, which is named 'footy' in the urls
    else:
...
def index(request):
    get_Info = Info.info.all()      #Gets all the current jerseys from the database
    context = {'Info': get_Info}      #Creates a dictionary object of all the jerseys for the template
